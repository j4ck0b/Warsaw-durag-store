...
function initCheckoutFlow() {
  ...
  placeOrderBtn.addEventListener('click', (e) => {
    ...
    setTimeout(() => {
      // Success Order Number generator
      const orderNo = Math.floor(100000 + Math.random() * 900000);
      
      const cartItemsContainer = document.getElementById('cartItemsContainer');
      const cartFooter = document.getElementById('cartFooter');
      
      // Hide multi-step forms
      cartStepCheckout.style.display = 'none';
      cartStepCart.style.display = 'flex';
      
      cartItemsContainer.style.opacity = '1';
      cartFooter.style.display = 'none'; // Hide inputs entirely
      
      // Show gorgeous success card
      cartItemsContainer.innerHTML = `
        <div class="checkout-success-view">
          ...
          <p style="font-size: 0.95rem; color: var(--color-secondary); line-height: 1.6; font-weight: 300;">
            Twoje zamówienie zostało pomyślnie przyjęte. Numer zamówienia: <strong style="color:var(--color-primary);">#WDS-${orderNo}</strong>. Szczegóły wysłaliśmy na e-mail: <strong style="color:var(--color-primary);">${emailVal}</strong>.
          </p>
      ...
      // Save order in history log for CMS Panel
      ...
      const newOrder = {
        ...
        rawItems: [...state.cart],
        shipping: {
          method: currentDeliveryMethod === 'paczkomat' ? 'Paczkomat InPost' : 'Kurier WDS',
          name: nameVal,
          email: emailVal,
          phone: phoneVal,
          lockerCode: currentDeliveryMethod === 'paczkomat' ? selectedPaczkomat.name : null,
          lockerAddress: currentDeliveryMethod === 'paczkomat' ? selectedPaczkomat.address : null
        }
      };
      
      const existingOrders = JSON.parse(localStorage.getItem('wds_orders') || '[]');
      existingOrders.unshift(newOrder); // Add to beginning of array
      localStorage.setItem('wds_orders', JSON.stringify(existingOrders));
      
      // Clear checkout state variables
      state.cart = [];
      state.promoApplied = null;
      selectedPaczkomat = null;
      ...
      // Refresh CMS statistics reactively if admin is logging in during checkout session
      ...
      document.getElementById('successCloseBtn').addEventListener('click', () => {
        ...
        // Restore default cart drawer layout for next session
        setTimeout(() => {
